...
func (pc *PodController) syncPodInProvider(ctx context.Context, pod *corev1.Pod) error {
	...
	if pod.DeletionTimestamp != nil {
		if err := pc.terminatePod(ctx, pod.Namespace, pod.Name); err != nil {
			err := pkgerrors.Wrapf(err, "failed to terminate pod %q in the provider", loggablePodName(pod))
			span.SetStatus(err)
			return err
		}
...
func (pc *PodController) terminateDanglingPods(ctx context.Context, threadiness int) {
	...
	// Iterate over the slice of pods to be removed in the provider and remove them.
	for _, pod := range ptd {
