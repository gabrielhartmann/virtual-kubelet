...
func (pc *PodController) terminatePod(ctx context.Context, namespace, name string) error {
	...
	if pod == nil {
		log.G(ctx).Warnf("The pod %q is unknown to the provider and cannot be terminated.", loggablePodNameFromCoordinates(namespace, name))
		return nil
	}
...
func (pc *PodController) deletePod(ctx context.Context, namespace, name string) error {
	...
	pod, _ := pc.provider.GetPod(ctx, namespace, name)
	if pod == nil {
		log.G(ctx).Warnf("The pod %q is unknown to the provider and cannot be deleted.", loggablePodNameFromCoordinates(namespace, name))
		return nil
	}
    ...
    delInKubernetes, delErr := pc.provider.DeletePod(ctx, pod)
	if delErr != nil && !errdefs.IsNotFound(delErr){
		span.SetStatus(delErr)
		return delErr
	}

	if delInKubernetes && !errors.IsNotFound(delErr){
